<div align="center">

<img src="https://i.pinimg.com/originals/22/f4/e1/22f4e155043a7e8485c5c7a8a71c680a.gif" width="60%">

# 𝐀𝐥𝐥 𝐇𝐚𝐢𝐥 𝐋𝐞𝐥𝐨𝐮𝐜𝐡

### Computer Science Student • Backend Developer • AI Enthusiast

> *“The world cannot be changed with pretty words alone.”*

</div>

---

# 🧠 About Me

```yaml
Name: Felipe Mariano
Role: Computer Science Student
Location: Brazil
Focus:
  - Backend Development
  - Artificial Intelligence
  - Cloud Computing
  - Automation

Currently Learning:
  - Docker
  - Git & GitHub
  - Spring Boot
  - AI Engineering
```

---

# Geass Abilities

<div align="center">

<img src="https://skillicons.dev/icons?i=java,spring,python,docker,git,github,mysql,postgresql,html,css,js" />

</div>

---

# Tactical Arsenal

```diff
+ Java & Spring Boot
+ Python Automation
+ SQL & Database Modeling
+ Front-End Development
+ Cloud Fundamentals
+ REST APIs
+ Git & Docker
+ Artificial Intelligence
```

---

# Black Knights Operations

## ⚔️ Operation Ragnarok

📌 Web system focused on performance and organization.

## 🤖 Geass AI

📌 Artificial intelligence chatbot project using Python.

## ☁️ Britannia Cloud Division

📌 Cloud computing studies and deployment practices.

## 🧩 Zero Protocol

📌 Backend API using Java + Spring Boot.

---

# Certifications

🏅 Cambridge English Certification — B1 Level
🏅 Python do Básico ao Avançado
🏅 Front-End Developer — SENAI
🏅 GFT Cloud Fundamentals Bootcamp
🏅 NTT DATA Java & Spring Boot Bootcamp

---

# GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=midnight-purple&hide_border=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=midnight-purple&hide_border=true"/>

</div>

---

# Contribution Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=SEU_USUARIO&theme=midnight-purple&hide_border=true" />

</div>

---

# Connect With Me

<div align="center">

<a href="https://www.linkedin.com/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white">
</a>

<a href="https://github.com/">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
</a>

</div>

---

# Final Protocol

<div align="center">

```diff
- Obedience without question is power.
+ Strategy is everything.
```

<img src="https://media.tenor.com/Wr6oflP0Y0UAAAAC/lelouch-geass.gif" width="500px">

## ALL HAIL LELOUCH

</div>
